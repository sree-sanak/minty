...
- **2026-05-17** — Confirmed the README no longer has a hero screenshot/GIF placeholder: it now renders the checked-in privacy-safe synthetic visual at `docs/hero.svg`, with alt text and caption explaining the OpenClaw/Hermes/MCP network-memory wedge. Remaining demo-readiness polish should now focus on keeping the quickstart and agent trust docs accurate, not regenerating a real-contact screenshot.
