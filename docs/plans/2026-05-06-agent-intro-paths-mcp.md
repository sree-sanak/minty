...
function targetMatchesByName(target, contacts, limit) {
    const q = String(target || '').trim().toLowerCase();
    if (!q) return [];
    return contacts
        .filter(c => c && !c.isGroup && c.id && c.name && c.name.toLowerCase().includes(q))
        .slice(0, limit);
}
...
function buildAgentIntroPaths(args = {}, data = {}) {
    ...
    const targets = args.goal
        ...
        : targetMatchesByName(args.target, contacts, limit);
...
```js
test('[MCP]: tools/list includes intro_paths', async () => {
    const names = TOOLS.map(t => t.name);
    assert.ok(names.includes('intro_paths'));
});

test('[MCP]: intro_paths returns redacted JSON envelope', async () => {
